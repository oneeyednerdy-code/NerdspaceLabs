...
export async function loadCreatorData(userId){
 ...
 const gameIds=[...clips.map(c=>c.game_id),channel?.game_id,...followedStreams.map(s=>s.game_id)].filter(Boolean);
 const games=await safe('twitch.games',()=>getGames(gameIds),[]);
 const trackerGames=await safe('twitchtracker.games',()=>getTwitchTrackerGameSummaries([...new Set(followedStreams.map(s=>s.game_id).filter(Boolean))],12),new Map());
 ...
 return {status,user,channel,stream,videos,followedStreams,followedChannels,clips,followerTotal,publishedSchedule,tracker,games,trackerGames,profiles,schedules};
}
