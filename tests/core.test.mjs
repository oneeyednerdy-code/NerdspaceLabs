...
test('version is visible on login loading and transition states',async()=>{const fs=await import('node:fs/promises');const html=await fs.readFile(new URL('../index.html',import.meta.url),'utf8');assert.ok((html.match(/1\.21\.6/g)||[]).length>=3);for(const id of ['authGate','workspaceLoading','workspaceTransition'])assert.ok(html.includes(`id="${id}"`))});
